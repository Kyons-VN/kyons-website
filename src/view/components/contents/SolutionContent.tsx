...
export function SolutionContent({l}:Props){
  ...
  return <div class='flex flex-row w-full h-full'>
    <div class="w-full md:w-[416px] h-full bg-lightOrange-4 md:bg-blueGrey-700 flex flex-col px-6">
      <div class='flex flex-col gap-6 pt-6 md:p-6 md:text-white md:mt-28'>
        <h5 class='text-orange'>{l.solution.ourSolution}</h5>
        <img class='md:hidden' src='images/Mock-Test/Group 1457.svg' />
        ...
        <span dangerouslySetInnerHTML={{__html: l.solution.registerNow}}></span>
          <div>
            <a class='btn' href="https://student.tuhoconline.org/sign-up" target='_blank' title={`Kyons App | ${l.signUp}`}>{l.solution.createAccount}</a>
          </div>
      </div>
    ...
    <div class='hidden md:flex w-full flex-1 h-full'>
      <ul class='inner'>
        ...
        <li className={$activeSolution==1?'active':null} onClick={()=>setActive(1)}>
          <img src={`images/${l.menu.lang}/learning-path.svg`} /></li>
