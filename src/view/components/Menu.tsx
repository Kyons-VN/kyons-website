...
export default function Menu({ l }: Props) {
  ...
  const defaultClass = 'md:absolute flex flex-col gap-2 py-[6px] px-4 rounded-lg shadow-2 bg-white text-black w-full md:left-0'
  ...
  return <nav class='menu' style={!isOpen&&isMobile?{maxHeight:'78px'}:{}}>
      <div className="flex flex-row justify-between">
        <img class='w-[100px] cursor-pointer' src='images/logo.svg' alt='Logo' onClick={()=>{setActiveMenu(0)}} />
        <button class='md:hidden text-[24px]' onClick={toggle}><i className={isMenuOpen.get()?'icon-Close':'icon-MenuHambuger'}></i></button>
      </div>
      <ul class='menu-items'>
      ...
      <div class='flex flex-row relative h-10 md:px-4 md:py-[10px] items-center justify-between text-sm md:w-24' onClick={() => setIsHover(!isHover)} onBlur={() => setIsHover(false)} tabIndex={0} >
        <span>{language}</span>
        <i className={isHover?'icon-ArrowUp':'icon-ArrowDown'}></i>
        <div className={isHover?`${defaultClass} hidden md:flex`:`${defaultClass} hidden`} style="bottom: calc(-100% - 30px);"  onClick={() => setIsHover(false)}>
      ...
      <div className={isHover?`${defaultClass} flex md:hidden`:`${defaultClass} hidden`} style="bottom: calc(-100% - 30px);"  onClick={() => setIsHover(false)}>
        <div className={language=='VI'?'text-orange cursor-pointer':'cursor-pointer'} onClick={()=>setLanguage('VI')}>VI</div>
        <hr class='text-blueGrey-300' />
        <div className={language=='EN'?'text-orange cursor-pointer':'cursor-pointer'} onClick={()=>setLanguage('EN')}>EN</div>
      </div>
